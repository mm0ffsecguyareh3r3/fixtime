...
def magenta(text):
    return color(text, Color.MAGENTA)


def dim(text):
    return color(text, Color.DIM)
...
def run_ntp_sync(server):
    ...
    if OS_NAME == "windows":
        ...
        for tool in ["ntpdate", "sntp", "chronyc"]:
...
def get_local_time_info():
    ...
    try:
        ...
        log(
            f"[-] Failed to get local time info: {e}"
        )
...
def auto_ntpdate_sync(ntp_server):
    ...
    cprint(
        f"\n[🔧] Running automatic NTP sync with "
        f"{cyan(ntp_server)}"
    )

    cprint(
        "[*] Using OS-specific NTP tools..."
    )

    disable_ntp()

    success_result = run_ntp_sync(
        ntp_server
    )
            ...
            try:
                ...
                if "Date" in r.headers:
                    ...
                    try:
                        ...
                        return (
                            remote_time,
                            f"WinRM (HTTP Date header) on {target}",
                        )
...
def get_remote_time_concurrent(
    url,
    host,
    ip=None,
):
...
def calculate_skew(
    ...
    return {
        ...
        "within_kerberos_tolerance": (
            skew <= KERBEROS_MAX_SKEW
        ),
...
def sync_time_manual(
    ...
    try:
        ...
        if remote_time_obj.tzinfo is None:
            ...
            remote_time_obj = (
                remote_time_obj.replace(
                    tzinfo=timezone.utc
                )
            )

        remote_utc = (
            remote_time_obj.astimezone(
                timezone.utc
            )
        )
        ...
        if (
            skew_info["within_kerberos_tolerance"]
            and not args.force
        ):
            return True
        ...
        if set_system_time(remote_utc):
            ...
            new_local = datetime.now(
                timezone.utc
            )

            new_skew = calculate_skew(
                new_local,
                remote_time_obj,
            )
        ...
        try:
            ...
            if result:
                ...
                if args.check_skew:
                    ...
                    print_value(
                        "Difference:",
                        f"{skew_info['absolute_seconds']:.2f} seconds",
                    )
